...
const addDocumentButton = document.querySelector<HTMLButtonElement>(".btn");
...
const leftPane = document.querySelector<HTMLDivElement>(".leftPane");
const logo = document.querySelector<HTMLDivElement>(".logo");
const user = document.querySelector<HTMLDivElement>(".user");
const icons = document.querySelector<HTMLDivElement>(".icons");
...
if (addDocumentButton && arrow && appDiv && profileButton && leftPane && logo && user && icons && bottom) {
  ...
  profileButton.addEventListener("click", (event: MouseEvent) => {
    event.stopPropagation();
    const profileModel = document.querySelector<HTMLDivElement>(".profileModel");
    if (profileModel) {
  ...
  document.addEventListener("click", (event: MouseEvent) => {
    const profileModel = document.querySelector<HTMLDivElement>(".profileModel");
    const profileButton = document.querySelector<HTMLButtonElement>(".profileButton");
    if (profileModel && !profileModel.contains(event.target as Node)) {
...
function handleArrow(): void {
  if (!arrow || !leftPane || !logo || !user || !icons || !bottom) return;
...
function addDocumentRow(event: Event): void {
  ...
  if (!fileNameInput || !lastModifiedInput || !statusSelect) return;

  const fileName = fileNameInput.value;
  const lastModified = lastModifiedInput.value;
  const status = statusSelect.value;

  const documentData: DocumentData = { fileName, lastModified, status };
...
function saveToLocalStorage(documentData: DocumentData): void {
  let documents: DocumentData[] = JSON.parse(localStorage.getItem("documents") || "[]");
  documents.push(documentData);
  localStorage.setItem("documents", JSON.stringify(documents));
}

function displayStoredDocuments(): void {
  const documents: DocumentData[] = JSON.parse(localStorage.getItem("documents") || "[]");
  documents.forEach((doc) => appendDocumentRow(doc));
}

function appendDocumentRow({ fileName, lastModified, status }: DocumentData): void {
